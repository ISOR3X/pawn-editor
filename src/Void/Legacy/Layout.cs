...
/// <summary>
///     Frame-scoped mutable wrapper around an immutable <see cref="ParsedLayoutLegacy" /> template tree.
...
/// </summary>
public class Layout(ParsedLayoutLegacy template, IContext? context = null)
{
    ...
    private XMLComponent GetConfigForNode(ParsedLayoutLegacy node)
    {
        return node.Id != null && _overrides.TryGetValue(node.Id, out var ov) ? ov : node.Props;
    }

    private void RenderNode(TaffyBuilder builder, ParsedLayoutLegacy node)
    {
