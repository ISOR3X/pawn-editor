...
/// </summary>
public class ParsedLayoutLegacy
{
    public readonly List<ParsedLayoutLegacy> Children = [];
    public string? Id;
        ...
        if (UnresolvedClasses is { Length: > 0 })
        {
            ...
            foreach (var className in UnresolvedClasses)
            {
                ...
                foreach (var styleDef in DefDatabase<StyleMapDef>.AllDefsListForReading)
                    if (styleDef.LegacyStyles.TryGetValue(className, out var s))
                    {
    ...
    public ParsedLayoutLegacy? FindById(string id)
    {
