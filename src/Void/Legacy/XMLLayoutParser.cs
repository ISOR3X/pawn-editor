...
/// <summary>
///     Parses a layout XML tag (<see cref="XmlNode" />) into a <see cref="ParsedLayoutLegacy" /> tree.
///     Called from <see cref="ParsedLayoutLegacy.LoadDataFromXmlCustom" />.
///     <para>
...
public static class XMLLayoutParser
{
    ...
    public static void Parse(XmlNode xmlNode, ParsedLayoutLegacy target)
    {
        ...
        if (!element.IsLeaf)
            foreach (XmlNode child in xmlNode.ChildNodes)
            {
                ...
                var childNode = new ParsedLayoutLegacy();
