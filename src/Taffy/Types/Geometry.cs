...
public struct Size<T>
{
    ...
    /// <summary>
    ///     Apply <paramref name="f" /> to Width and Height paired with the corresponding element of
    ///     <paramref name="other" />.
    /// </summary>
    ...
    public T Main(FlexDirection dir)
    {
        return dir.IsRow() ? Width : Height;
    }

    public T Cross(FlexDirection dir)
    {
        return dir.IsRow() ? Height : Width;
    }
    ...
    {
        return dir.IsRow() ? new Size<T>(value, Height) : new Size<T>(Width, value);
    }

    public Size<T> WithCross(FlexDirection dir, T value)
    {
        return dir.IsRow() ? new Size<T>(Width, value) : new Size<T>(value, Height);
    }

    public Size<T> MapMain(FlexDirection dir, Func<T, T> f)
    {
        return dir.IsRow() ? new Size<T>(f(Width), Height) : new Size<T>(Width, f(Height));
    }

    public Size<T> MapCross(FlexDirection dir, Func<T, T> f)
    {
        return dir.IsRow() ? new Size<T>(Width, f(Height)) : new Size<T>(f(Width), Height);
    }
    ...
    {
        return axis == AbstractAxis.Inline ? new Size<T>(value, Height) : new Size<T>(Width, value);
    }

    /// <summary>Returns a new Size with the given axis set to <paramref name="value" />.</summary>
    public Size<T> WithAxisAbs(AbsoluteAxis axis, T value)
    {
        return axis == AbsoluteAxis.Horizontal ? new Size<T>(value, Height) : new Size<T>(Width, value);
    }
...
{
    public static readonly Size<float> ZERO = new(0f, 0f);

    /// <summary>A <see cref="Size{T}" /> with <c>None</c> width and height.</summary>
    public static readonly Size<float?> NONE = new(null, null);

    public static Size<float> Add(in Size<float> a, in Size<float> b)
    {
        return new Size<float>(a.Width + b.Width, a.Height + b.Height);
    }
    ...
    {
        return new Size<float>(MathF.Max(a.Width, b.Width), MathF.Max(a.Height, b.Height));
    }

    public static Size<float> Min(in Size<float> a, in Size<float> b)
    {
        return new Size<float>(MathF.Min(a.Width, b.Width), MathF.Min(a.Height, b.Height));
    }

    public static bool HasNonZeroArea(in Size<float> s)
    {
        return s.Width > 0f && s.Height > 0f;
    }
    ...
    {
        return new Size<float>(s.Width ?? alt.Width, s.Height ?? alt.Height);
    }

    public static bool BothAxisDefined(in Size<float?> s)
    {
        return s.Width.HasValue && s.Height.HasValue;
    }
...
public struct Rect<T>
{
    ...
    {
        return new Rect<R>(f(Left, size.Width), f(Right, size.Width), f(Top, size.Height), f(Bottom, size.Height));
    }
    ...
    // Flexbox axis helpers
    public T MainStart(FlexDirection dir)
    {
        return dir.IsRow() ? Left : Top;
    }

    public T MainEnd(FlexDirection dir)
    {
        return dir.IsRow() ? Right : Bottom;
    }

    public T CrossStart(FlexDirection dir)
    {
        return dir.IsRow() ? Top : Left;
    }

    public T CrossEnd(FlexDirection dir)
    {
        return dir.IsRow() ? Bottom : Right;
    }

    public override string ToString()
    {
        return $"Rect(L={Left}, R={Right}, T={Top}, B={Bottom})";
    }
}
...
{
    public static readonly Rect<float> ZERO = new(0f, 0f, 0f, 0f);

    public static float HorizontalAxisSum(in Rect<float> r)
    {
        return r.Left + r.Right;
    }

    public static float VerticalAxisSum(in Rect<float> r)
    {
    ...
    {
        return new Size<float>(HorizontalAxisSum(r), VerticalAxisSum(r));
    }

    public static float MainAxisSum(in Rect<float> r, FlexDirection dir)
    {
        return dir.IsRow() ? HorizontalAxisSum(r) : VerticalAxisSum(r);
    }

    public static float CrossAxisSum(in Rect<float> r, FlexDirection dir)
    {
        return dir.IsRow() ? VerticalAxisSum(r) : HorizontalAxisSum(r);
    }

    public static float GridAxisSum(in Rect<float> r, AbsoluteAxis axis)
    {
        return axis == AbsoluteAxis.Horizontal ? HorizontalAxisSum(r) : VerticalAxisSum(r);
    }

    public static Rect<float> Add(in Rect<float> a, in Rect<float> b)
    {
        return new Rect<float>(a.Left + b.Left, a.Right + b.Right, a.Top + b.Top, a.Bottom + b.Bottom);
    }
}
...
public struct Point<T>
{
    ...
    public T Main(FlexDirection dir)
    {
        return dir.IsRow() ? X : Y;
    }

    public T Cross(FlexDirection dir)
    {
        return dir.IsRow() ? Y : X;
    }
...
{
    public static readonly Point<float> ZERO = new(0f, 0f);
    public static readonly Point<float?> NONE = new(null, null);
...
{
    public static readonly Line<bool> TRUE = new(true, true);
    public static readonly Line<bool> FALSE = new(false, false);

    public static float Sum(in Line<float> l)
    {
