...
internal static class GridAlignment
{
    ...
    {
        var usedSize = 0f;
        for (var i = 0; i < tracks.Count; i++) usedSize += tracks[i].BaseSize;

        var freeSpace = gridContainerContentBoxSize - usedSize;
        var origin = padding.Start + border.Start;

        // Count non-collapsed actual tracks (odd indices, since even = gutters)
        var numTracks = 0;
        for (var i = 1; i < tracks.Count; i += 2)
        ...
        var seenNonCollapsed = false;

        for (var i = 0; i < tracks.Count; i++)
        {
            var track = tracks[i];
            var isGutter = i % 2 == 0;
            var isNonCollapsed = !isGutter && !track.IsCollapsed;
            var isFirst = isNonCollapsed && !seenNonCollapsed;

            var offset = isNonCollapsed
        ...
        Direction direction)
    {
        ...
        var insetHorizontal = style.inset.HorizontalComponents().Map(lpa => lpa.MaybeResolve(gridAreaSize.Width));
        var insetVertical = style.inset.VerticalComponents().Map(lpa => lpa.MaybeResolve(gridAreaSize.Height));

        var padding = style.padding.ResolveOrZero(gridAreaSize.Width);
        var border = style.border.ResolveOrZero(gridAreaSize.Width);
        ...
        var finalWidth = (width ?? layoutOutput.Size.Width).MaybeClamp(minSize.Width, maxSize.Width);
        var finalHeight = (height ?? layoutOutput.Size.Height).MaybeClamp(minSize.Height, maxSize.Height);
        ...
        Direction direction)
    {
        ...
        var gridAreaSize = MathF.Max(gridArea.End - gridArea.Start, 0f);
        var freeSpace = MathF.Max(
            gridAreaSize - resolvedSize - nonAutoMargin.Start - nonAutoMargin.End, 0f);

        var autoCount = (margin.Start.HasValue ? 0 : 1) + (margin.End.HasValue ? 0 : 1);
        var autoSize = autoCount > 0 ? freeSpace / autoCount : 0f;
        ...
        var alignmentOffset = alignmentStyle switch
        {
        ...
        var startPos = gridArea.Start + offsetWithinArea;
        if (position == Position.Relative)
        {
            var relInset = isRtl
                ? inset.End.HasValue ? -inset.End.Value : inset.Start
                : inset.Start.HasValue
                    ? inset.Start
                    : inset.End.HasValue
                        ? -inset.End.Value
                        : null;
