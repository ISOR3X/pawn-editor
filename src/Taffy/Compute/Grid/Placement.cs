...
internal static class Placement
{
    ...
    {
        return direction == Direction.Rtl && axis == AbsoluteAxis.Horizontal;
    }
    ...
    {
        return reversed ? new OriginZeroLine((short)(gridEnd.Value - 1)) : gridStart;
    }
        ...
        Style containerStyle)
    {
        ...
        var explicitColCount = occupancy.TrackCounts(AbsoluteAxis.Horizontal).Explicit;
        var explicitRowCount = occupancy.TrackCounts(AbsoluteAxis.Vertical).Explicit;
            ...
            ushort explicitColCount)
    {
        ...
        else
        {
            ...
            if (primaryAxisIsReversed)
                lookupResult =
                    occupancy.FirstOfType(primaryAxis, secondarySpan.Start, CellOccupancyState.AutoPlaced);
            else
                lookupResult =
                    occupancy.LastOfType(primaryAxis, secondarySpan.Start, CellOccupancyState.AutoPlaced);
            startingPos = lookupResult ?? SearchStartLine(primaryGridStart, primaryGridEnd, primaryAxisIsReversed);
        }

        var primarySpanCount = placement.Get(primaryAxis).IndefiniteSpan();
        var pos = startingPos;
        while (true)
        {
            ...
            ushort explicitColCount)
    {
        ...
        var hasDefinitePrimary = primaryPlacement.IsDefinite();
        ...
        if (hasDefinitePrimary)
        {
            ...
            {
                var shouldAdvance = primaryAxisIsReversed
            ...
            {
                var secondarySpan =
                    ResolveIndefiniteSpan(secondaryIdx, secondarySpanCount, secondaryAxisIsReversed);
                if (occupancy.LineAreaIsUnoccupied(primaryAxis, primarySpan, secondarySpan))
                    return (primarySpan, secondarySpan);
                secondaryIdx = AdvancePosition(secondaryIdx, secondaryAxisIsReversed);
            }
        ...
        while (true)
        {
            ...
            var primaryOutOfBounds = primaryAxisIsReversed
