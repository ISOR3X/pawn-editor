...
internal static class FlexCompute
{
        ...
        TaffyTree tree, ref AlgoConstants c, Size<AvailableSpace> available, FlexItem[] items)
    {
        ...
        for (var idx = 0; idx < items.Length; idx++)
        {
            ...
            var hypoInner = child.flexBasis.MaybeClamp(
                hypoInnerMin, child.maxSize.Main(c.dir));
        ...
        ref AlgoConstants c, Size<AvailableSpace> available, FlexItem[] items)
    {
        ...
        if (!c.isWrap) return [new FlexLine { Items = items, Start = 0, Count = items.Length }];
        ...
        if (mainAS.IsMaxContent) return [new FlexLine { Items = items, Start = 0, Count = items.Length }];
        ...
        for (var i = line.Start; i < line.Start + line.Count; i++)
        {
            ...
            else
            {
                var knownForMeasure = dir.IsRow()
                    ? new Size<float?>(child.targetSize.Width, null)
                    : new Size<float?>(null, child.targetSize.Height);
    ...
    {
        return numItems <= 1 ? 0f : gap * (numItems - 1);
    }
