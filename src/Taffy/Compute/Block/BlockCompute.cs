...
internal sealed class BlockContext
{
    ...
    public BlockContext SubContext()
    {
        return new BlockContext(false);
    }
}
...
internal static class BlockCompute
{
        ...
        BlockContext blockCtx)
    {
        ...
        var ownMarginsCollapseWithChildren = new Line<bool>(
            vertMarginCollapsible.Start
            && !isScrollContainer
            && style.position == Position.Relative
            && padding.Top == 0f
            && border.Top == 0f,
            vertMarginCollapsible.End
            && !isScrollContainer
            && style.position == Position.Relative
            && padding.Bottom == 0f
            && border.Bottom == 0f
        ...
        var resolvedPadding = rawPadding.ResolveOrZero(containerOuterWidth);
        var resolvedBorder = rawBorder.ResolveOrZero(containerOuterWidth);
        ...
        out CollapsibleMarginSet lastChildBottomMarginSet)
    {
        ...
        for (var idx = 0; idx < items.Count; idx++)
        {
            ...
            var itemMargin = item.margin.MaybeResolve(containerOuterWidth);
            ...
            if (item.isInSameBfc)
            {
                ...
                var subCtx = new BlockContext(false);
            ...
            var resolvedMargin = new Rect<float>(
                itemMargin.Left ?? autoMarginSize,
                itemMargin.Right ?? autoMarginSize,
                topMarginSet.Resolve(),
                bottomMarginSet.Resolve());

            // Resolve inset.
            var insetLeft = item.inset.Left.MaybeResolve(containerInnerWidth) ?? 0f;
            var insetRight = item.inset.Right.MaybeResolve(containerInnerWidth) ?? 0f;
            var insetTop = item.inset.Top.MaybeResolve(0f) ?? 0f;
            var insetBottom = item.inset.Bottom.MaybeResolve(0f) ?? 0f;
            var insetOffset = new Point<float>(
                direction.IsRtl()
                    ? item.inset.Right.MaybeResolve(containerInnerWidth).HasValue
                        ? -insetRight
                        : insetLeft
                    : item.inset.Left.MaybeResolve(containerInnerWidth) ?? -insetRight,
                insetTop - insetBottom != 0f ? insetTop : 0f);
        ...
        Size<float> areaSize, Point<float> areaOffset, Direction direction)
    {
        ...
        for (var i = 0; i < items.Count; i++)
        {
            ...
            var aspectRatio = childStyle.aspectRatio;
            var margin = childStyle.margin.MaybeResolve(areaWidth);
            var padding = childStyle.padding.ResolveOrZero(areaWidth);
            var border = childStyle.border.ResolveOrZero(areaWidth);
            var pbSum = RectF.SumAxes(RectF.Add(padding, border));
            var bsAdj = childStyle.boxSizing == BoxSizing.ContentBox ? pbSum : SizeF.ZERO;

            var left = childStyle.inset.Left.MaybeResolve(areaWidth);
            var right = childStyle.inset.Right.MaybeResolve(areaWidth);
            var top = childStyle.inset.Top.MaybeResolve(areaHeight);
            var bottom = childStyle.inset.Bottom.MaybeResolve(areaHeight);
            ...
            var nonAutoMargin = new Rect<float>(
                left.HasValue ? margin.Left ?? 0f : 0f,
                right.HasValue ? margin.Right ?? 0f : 0f,
                top.HasValue ? margin.Top ?? 0f : 0f,
                bottom.HasValue ? margin.Bottom ?? 0f : 0f);
            ...
            var resolvedMargin = new Rect<float>(
                margin.Left ?? autoMarginW,
                margin.Right ?? autoMarginW,
                margin.Top ?? autoMarginH,
                margin.Bottom ?? autoMarginH);
...
internal static class RectFloatNullableExt
{
    ...
    {
        return (r.Left ?? 0f) + (r.Right ?? 0f);
    }

    public static Rect<float> map(this Rect<float?> r, Func<float?, float> f)
    {
        return new Rect<float>(f(r.Left), f(r.Right), f(r.Top), f(r.Bottom));
    }
}
...
internal static class SizeFloatNullableExt
{
    ...
    {
        return SizeF.MaybeApplyAspectRatio(self, aspectRatio);
    }

    public static Size<float> MaybeClamp(this Size<float> self, Size<float?> min, Size<float?> max)
    {
        return new Size<float>(self.Width.MaybeClamp(min.Width, max.Width),
