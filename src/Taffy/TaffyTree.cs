...
public sealed class TaffyTree
{
    ...
    {
        return AllocNode(style, null);
    }

    /// <summary>Creates a new leaf node with an attached context object (e.g. a measure function).</summary>
    public NodeId NewLeafWithContext(Style style, object context)
    {
        return AllocNode(style, context);
    }
    ...
    public void Remove(NodeId node)
    {
        var id = node.Value;
        // Detach from parent
        if (_parents[(int)id] is NodeId parent) _children[(int)parent.Value].Remove(node);
    ...
    {
        return _nodes[(int)node.Value].Style;
    }
    ...
    public object? GetContext(NodeId node)
    {
        return _nodes[(int)node.Value].Context;
    }

    public void SetContext(NodeId node, object? context)
    {
        _nodes[(int)node.Value].Context = context;
    }

    // ── Layout accessors ──────────────────────────────────────────────────

    /// <summary>Returns the computed layout of a node. Must call <see cref="ComputeLayout" /> first.</summary>
    public ref Layout Layout(NodeId node)
    {
        return ref _nodes[(int)node.Value].FinalLayout;
    }

    // ── Tree traversal helpers ────────────────────────────────────────────

    public IReadOnlyList<NodeId> Children(NodeId node)
    {
        return _children[(int)node.Value];
    }

    public int ChildCount(NodeId node)
    {
        return _children[(int)node.Value].Count;
    }

    public NodeId ChildAt(NodeId node, int index)
    {
        return _children[(int)node.Value][index];
    }

    public NodeId? Parent(NodeId node)
    {
        return _parents[(int)node.Value];
    }
    ...
    {
        return _nodes[(int)node.Value].IsDirty;
    }
    ...
    ///         <c>
    ///             (knownDimensions, availableSpace, nodeId,
    ///             nodeContext, style)
    ///         </c>
    ///         for every leaf node and returns its natural size. Pass <c>null</c>
    ...
    private bool IsLeaf(NodeId node)
    {
        return _children[(int)node.Value].Count == 0;
    }
    ...
    /// </summary>
    public void RoundLayout(NodeId root)
    {
        RoundLayout(root, 0f, 0f);
    }
    ...
    private void RoundLayout(NodeId node, float cumulativeX, float cumulativeY)
    {
        ...
        layout.Location.X = MathF.Round(absX, MidpointRounding.AwayFromZero) -
                            MathF.Round(cumulativeX, MidpointRounding.AwayFromZero);
        layout.Location.Y = MathF.Round(absY, MidpointRounding.AwayFromZero) -
                            MathF.Round(cumulativeY, MidpointRounding.AwayFromZero);
        layout.Size.Width = MathF.Round(absX + layout.Size.Width, MidpointRounding.AwayFromZero) -
                            MathF.Round(absX, MidpointRounding.AwayFromZero);
        layout.Size.Height = MathF.Round(absY + layout.Size.Height, MidpointRounding.AwayFromZero) -
                             MathF.Round(absY, MidpointRounding.AwayFromZero);
    ...
    {
        ref var layout = ref _nodes[(int)node.Value].FinalLayout;
        layout = Taffy.Layout.WithOrder(order);
    ...
    internal NodeData GetNodeData(NodeId node)
    {
        return _nodes[(int)node.Value];
    }
}