...
#pragma warning disable CS0649 // Fields assigned by grid algorithm methods not yet in this file
    ...
    internal sealed class GridItem
    {
        ...
        private static Size<AvailableSpace> MapToAvailableSpace(Size<float?> s, bool minContent)
        {
            AvailableSpace Map(float? opt) =>
                opt.HasValue ? TaffySharp.AvailableSpace.Definite(opt.Value)
                    : (minContent ? TaffySharp.AvailableSpace.MinContent : TaffySharp.AvailableSpace.MaxContent);
