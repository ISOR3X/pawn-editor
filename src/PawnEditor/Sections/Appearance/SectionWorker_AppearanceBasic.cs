...
public class SectionWorker_AppearanceBasic(SectionDef def) : SectionWorker(def)
{
    protected override void DoSectionContents(Listing_Standard listing, Pawn pawn)
    {
        ...
        Verse.Widgets.ButtonText(sexRect, pawn.gender.GetLabel().CapitalizeFirst());

        var lifestageRect = listing.RectLabeled("Lifestage");
        Verse.Widgets.ButtonText(lifestageRect, pawn.DevelopmentalStage.ToString());

        if (!ModsConfig.BiotechActive) return;

        var xenotypeRect = listing.RectLabeled("Xenotype");
        Verse.Widgets.ButtonText(xenotypeRect, pawn.genes.XenotypeLabelCap);

        var races = GetRacesForPawn(pawn);
        if (races.NullOrEmpty()) return;

        var raceRect = listing.RectLabeled("Race");
        Verse.Widgets.ButtonText(raceRect, pawn.kindDef.race.LabelCap);
    }
