...
public static class AppearanceUtility
{
    ...
    public static bool CanUseHeadType(HeadTypeDef headTypeDef, Pawn pawn)
    {
        if (ModsConfig.BiotechActive && !headTypeDef.requiredGenes.NullOrEmpty())
        {
            ...
            if (headTypeDef.requiredGenes.Any(requiredGene => !pawn.genes.HasGene(requiredGene)))
            {
                return false;
            }
        }

        if (headTypeDef.gender != 0)
        {
            return headTypeDef.gender == pawn.gender;
        }
    ...
    private static List<BodyTypeDef> AllBodyTypesFor(Pawn pawn)
    {
        ...
        List<BodyTypeDef> bodyTypeDefList = new List<BodyTypeDef>();
        if (ModsConfig.BiotechActive && pawn.DevelopmentalStage.Juvenile())
            bodyTypeDefList.Add(pawn.DevelopmentalStage == DevelopmentalStage.Baby ? BodyTypeDefOf.Baby : BodyTypeDefOf.Child);
        if (ModsConfig.BiotechActive && pawn.genes != null)
        {
            List<Gene> genesListForReading = pawn.genes.GenesListForReading;
            bodyTypeDefList.AddRange(from t in genesListForReading where t.def.bodyType.HasValue select t.def.bodyType.Value.ToBodyType(pawn));
        }
