...
/// <summary>
/// Abstract base for XML-driven column workers whose rows are <see cref="Def"/> instances.
/// Subclasses read sizing, header text, and tooltip from the <see cref="ColumnDef"/> assigned
/// by <see cref="DefColumnDef.Worker"/> during <c>ResolveReferences</c>.
/// </summary>
public abstract class DefColumnWorker : ColumnWorker<Def>
{
    ...
    /// this in a grid item. For full control over the grid item (e.g., push multiple items),
    /// override <see cref="DrawCell"/> directly instead.
    /// </summary>
    protected virtual void DrawCellContent(Rect r, Def row) { }
