...
//     }
//
//     // Returns the worker cast to ColumnWorker<T>, throwing clearly if a column is misconfigured.
//     private ColumnWorker<T> GetTypedWorker(ColumnDef col)
//         => (ColumnWorker<T>)col.Worker;
//
...
//             Verse.Widgets.DrawLineHorizontal(position.x, position.y + _cachedHeaderHeight, num2);
//
...
//                          index3 < _cachedThings.Count && GetTypedWorker(_cachedColumns[columnIndex])
...
//     }
//
//     protected virtual void DoRow(ColumnDef columnDef, Rect rect, T item)
//     {
//         GetTypedWorker(columnDef).DoCell(rect, item, this);
//     }
//
...
//
//     protected virtual void DoRowHover(Rect inRect, T item)
//     {
//     }
//
...
//         _cachedThings.Clear();
//
//         // if (_def.searchColumn?.Worker is ColumnWorker_Text<T> col && _quickSearchWidget.filter.Text != null)
//         //     _cachedThings.AddRange(_thingsGetter().Where(t =>
//         //     {
//         //         var text = col.GetTextFor(t);
//         //         return text != null && text.ToLower().Contains(_quickSearchWidget.filter.Text.ToLower());
//         //     }));
//         // else
...
//         {
//             if (_sortDescending)
//                 _cachedThings.SortStable((a, b) => GetTypedWorker(SortingBy).Compare(a, b));
//             else
//                 _cachedThings.SortStable((a, b) => GetTypedWorker(SortingBy).Compare(b, a));
//         }
//
//         _cachedThings = PrimarySortFunction(_cachedThings).ToList();
//     }
//     
//     protected abstract IEnumerable<T> LabelSortFunction(IEnumerable<T> input);
//     
...
//
//     private float CalculateRowHeight(T item)
//     {
//         return _cachedColumns.Aggregate(_def.defaultRowHeight, (current, col) => Mathf.Max(current, GetTypedWorker(col).GetMinCellHeight(item)));
//     }
//
//     private float CalculateHeaderHeight()
//         => _cachedColumns.Aggregate(0.0f, (current, t) => Mathf.Max(current, t.Worker.GetMinHeaderHeight(this)));
//
//     private float CalculateTotalRequiredHeight()
//         => CalculateHeaderHeight() + _cachedThings.Sum(CalculateRowHeight);
//
//     #region Properties
//
...
// }
//
// public abstract class DefTable(TableDef def, Func<IEnumerable<Def>> thingsGetter, Def? defaultThing = null)
//     : Table<Def>(def, thingsGetter, defaultThing)
// {
//     protected override IEnumerable<Def> LabelSortFunction(IEnumerable<Def> input)
//         => input.OrderBy(p => p.label);
// }
//
// public abstract class ThingTable(TableDef def, Func<IEnumerable<Thing>> thingsGetter, Thing? defaultThing = null)
//     : Table<Thing>(def, thingsGetter, defaultThing)
// {
