...
public class RowFilter_SpawnCategory()
    : RowFilter_ToggleableList<BackstoryDef, string>("Spawn category", s => s.ReadableCamelCase(), [])
{
    ...
    {
        return row.spawnCategories.Count > 0 ? row.spawnCategories : null;
    }
}