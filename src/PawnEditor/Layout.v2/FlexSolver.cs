...
public static class FlexSolver
{
    ...
    public static void Compute(LayoutContainer container, Rect availableRect)
    {
        ...
        if (!canWrap)
        {
            var line = ComputeLine(items, mainSize, crossSize, isRow, itemGap);
        ...
        {
            var lines = WrapIntoLines(items, mainSize, crossSize, isRow, itemGap);
            var crossOffset = 0f;
        ...
        bool isRow, float gap)
    {
        ...
        foreach (var item in items)
        {
            ...
            {
                lines.Add(ComputeLine(current, mainSize, crossSize, isRow, gap));
    ...
    {
        return length.IsAuto ? fallback : length.ResolveOr(parentSize, fallback);
    }
