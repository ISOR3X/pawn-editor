...
namespace FlexLayout
{
    ...
    /// Container-specific layout properties (direction, wrap, gap) live on <see cref="Style"/>.
    /// </summary>
    public class LayoutContainer
    {
        private readonly List<LayoutElement> _children = new List<LayoutElement>();

        public IReadOnlyList<LayoutElement> Children => _children;

        public ElementStyle Style { get; set; }

        /// <summary>The resolved rect for this container, set during layout.</summary>
        public Rect ComputedRect { get; internal set; }

        public void Add(LayoutElement child)    => _children.Add(child);
        public void Remove(LayoutElement child) => _children.Remove(child);
