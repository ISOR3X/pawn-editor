...
public static class PawnLister
{
    ...
    private static readonly List<Pawn> PawnsNoFactionTemporary = new();
    private static readonly List<PawnLocation> AllLocationsTemporary = new();
    ...
    public static List<PawnLocation> AllLocations
    {
        get
        {
            ...
            AllLocationsTemporary.AddRange(PawnsFinder.All_AliveOrDead.Select(p => p.GetLocation()).Distinct().OrderBy(l => l.Label));
    ...
    public static (Dictionary<Faction, List<Pawn>>, List<Pawn>) Pawns_ByFaction
    {
        get
        {
            ...
            PawnsByFactionTemporary.AddRange(Find.FactionManager.allFactions.ToDictionary(f => f, _ => new List<Pawn>()));
            foreach (var group in PawnsFinder.All_AliveOrDead
                         .Where(p => (!(p.IsWorldPawn() && Find.WorldPawns.GetSituation(p) == WorldPawnSituation.Dead)) || !PawnEditorMod.Settings.HideDeadWorldPawns)
                         .GroupBy(p => p.Faction))
            {
                if (group.Key != null)
                {
